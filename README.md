...
```
┌───────────┐   POST /api/analyses    ┌────────────┐   BullMQ job    ┌────────────┐
│ Frontend  │ ──────────────────────▶ │ API server │ ──────────────▶ │  Worker    │
│ (React)   │ ◀── SSE / polling ───── │ (Express)  │                 │ (pipeline) │
└───────────┘                         └─────┬──────┘                 └─────┬──────┘
                                            │  reads/writes                │ writes
                                            ▼                              ▼
                                   ┌─────────────────────────────────────────┐
                                   │  workspace/<analysis-id>/  (filesystem)  │
                                   │  metadata · progress · report · evidence │
                                   │  logs · exports/  (repositories/ deleted)│
                                   └─────────────────────────────────────────┘
```
...
Open http://localhost:5173, paste a repository URL, and start an analysis.
